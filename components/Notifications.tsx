    ...
    return (
        <Popover>
            <PopoverTrigger className='relative flex size-10 items-center justify-center rounded-lg'>
                ...
                {count > 0 && <div className='absolute top-2 right-2 size-2 rounded-full z-20 bg-blue-500 text-sm'></div>
                }
            </PopoverTrigger>
            <PopoverContent align='end' className='shad-popover'>
                ...
                >
                    <InboxNotificationList>
                        {unreadNotifications.length <= 0 ? (<p className='py-2 text-center text-dark-500'>No new notifications</p>) : unreadNotifications.map((notification) => (
                            <InboxNotification key={notification.id} inboxNotification={notification}
                                ...
                                kinds={{
                                    ...
                                    textMention: (props) => <InboxNotification.TextMention {...props}
                                        showRoomName={false}
                                    />,
                                    $documentAccess: (props) => <InboxNotification.Custom {...props}
                                        title={props.inboxNotification.activities[0].data.title}
                                        aside={<InboxNotification.Icon className='bg-transparent'>
                                            <Image src={props.inboxNotification.activities[0].data.avatar as string} alt="avatar" width={36} height={36}
